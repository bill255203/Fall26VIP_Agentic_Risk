...
Discuss authorship expectations near the start of the work and revisit them as contributions change. Track contributions in Individual tasks and linked PRs, using plain descriptions of each person’s part. The final group report includes a short evidence-linked contribution summary reviewed by contributors. [CRediT contributor roles](https://credit.niso.org/) are optional for later publication; students need no separate taxonomy or portfolio log. Sourced question exploration, substantive critique, reproduction attempts, and evidence-backed recommendations to reject a proposal can all be substantive contributions.
