...
| --- | --- | --- |
| **Pair case** | One pair's workflow, question, plan, report links, and milestone checklist for the semester. Create after partners agree; register its link in #3. | Names both partners and a reviewer. Both may be assigned. Stays open until the case outputs are reviewed. Assignment alone does not allocate individual credit. |
...
1. Onboard with your Individual task under #5 and starter task under #1.
2. When partners are confirmed, **one partner** opens **New issue → Pair case**,
   names both partners, and links it in #3. A maintainer confirms membership, reviewer,
...
6. Write your individual report under #23 and link it from the Pair case as well.

A task has **one primary parent**. Related goals may link it without requiring a
second task. Shared resource work can belong directly to #2, #6, or #7 when agreed;
...
For example, starter critiques from several students belong under #1.
...
| --- | --- | --- | --- |
| Roster and onboarding | #5 | #5; M1 Sep 25 | Access/attribution; setup alone is not research credit |
| Starter attempt and critique | #1 | #1; M1 Sep 25 | Foundation evidence |
| Reading / business workflow | Pair case (or #2 for shared resources) | #2 resources; M2 Oct 9 | Foundation 10 |
| Question and evidence plan | Pair case | #3 case registry; outline M2, reviewed plan M3 Oct 16 | Question 10 |
| Guided platform / trace package | Instructor #6; agreed student help may have a task under #6 | #6 before requiring platform use | Student credit only for actual agreed work |
| Feasibility and execution | Pair case | #7 validation; M3 then M4 | Execution 20 |
| Analysis | Pair case | #8 first analysis; M4 Oct 30 | Execution or Synthesis; distinct outputs, no double counting |
| Midterm case/question presentation | Pair case; reuse design task if it covers the role | #21 collects each pair's deck/feedback; M3 Oct 16 | Pair 10 |
| Final report and reproducible evidence | Pair case | #9; draft Nov 6, final M5 Nov 20 | Synthesis 10 individually; integrated pair package 20 |
| Final presentation | Pair case; reuse report task if it covers the role | #22; M5 Nov 20 | Required, feedback only; no separate weight |
| Individual contribution report | #23; cross-link Pair case | #23; draft Nov 6, final Nov 20 | Individual 10 |
...
| Course policy, support, private assessment | Instructor #24; students do not open tasks for course administration | Checkpoints through official cutoff | Private instructor assessment |
| Candidate A practical gate | Pair case only if explicitly adopted; related #4 | #4 deferred, no delivery milestone | Optional scope; no additional points |
...
cohort's #3/#7 to close. A maintainer closes collection goals only after every required
...
Existing student #28, #30, and PR #29 remain valid. Do not rewrite student-authored
...
and output collections; #4 stays optional. No pair membership or completed result is
inferred from an old assignment. Confirm partners in #3 before creating a Pair case.
